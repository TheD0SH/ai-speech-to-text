...
config_data = {
    ...
    # Audio settings
    "noise_threshold": 0.01,  # Minimum audio level to start recording (0.0-1.0)
    "recording_delay": 0.0,  # Delay before starting transcription (seconds)
    "auto_punctuation": True,  # Auto-add punctuation via Groq
    ...
    "auto_save_transcriptions": True,  # Auto-save all transcriptions
...
NOISE_THRESHOLD = config_data.get("noise_threshold", 0.01)  # Minimum audio level to start
RECORDING_DELAY = config_data.get("recording_delay", 0.0)  # Delay before transcription
AUTO_PUNCTUATION = config_data.get("auto_punctuation", True)  # Auto-punctuation via Groq
MAX_HISTORY = config_data.get("max_history", 100)  # Maximum history entries
AUTO_SAVE_TRANSCRIPTIONS = config_data.get("auto_save_transcriptions", True)  # Auto-save all transcriptions
...
class FloatingWidget:
    ...
    def apply_theme(self, theme_name):
        ...
        self.hint_label = tk.Label(
            self.content_frame,
            text=f"Press {HOTKEY.upper()} to record | Right-click tray for settings | v{__version__}",
            font=("Segoe UI", 8),
            fg=self.text_secondary,
            bg=self.bg_medium,
        )
    ...
    def open_settings(self):
        ...
        content = tk.Frame(scrollable_frame, bg=self.bg_dark, padx=25, pady=15)
        ...
        lang_options = ["auto", "en", "es", "fr", "de", "it", "pt", "ru", "ja", "ko", "zh", "ar", "hi", "nl", "pl", "tr", "el", "sq", "he", "fa", "ur", "vi", "th", "id", "ms", "cs", "sv", "da", "no", "fi"]
        lang_combo = ttk.Combobox(lang_frame, textvariable=language_var, 
                                   values=lang_options, state="readonly", width=20)
        lang_combo.pack(side=tk.LEFT)
        tk.Label(lang_frame, text="  Auto = detect automatically | el=Greek, sq=Albanian", 
                bg=self.bg_dark, fg=self.text_secondary, font=("Segoe UI", 9)).pack(side=tk.LEFT)
...
def transcribe_audio_file():
    ...
    if len(file_paths) == 1:
        _transcribe_single_file(file_paths[0])
    else:
        _transcribe_batch_files(file_paths)


def _transcribe_single_file(file_path):
...
def _transcribe_batch_files(file_paths):
    ...
    tk.Label(batch_win, text=f"📝 Batch Transcription ({len(file_paths)} files)", 
            font=("Segoe UI", 14, "bold"), bg="#1a1a2e", fg="#4a9eff").pack(pady=15)
    ...
    progress_text = tk.Text(progress_frame, height=15, bg="#16213e", fg="#ffffff",
    ...
    def process_files():
        for i, file_path in enumerate(file_paths, 1):
            ...
            progress_text.insert(tk.END, f"[{i}/{len(file_paths)}] Processing: {filename}\n")
            ...
            if text:
                ...
                progress_text.insert(tk.END, f"  ✅ {word_count} words: {text[:50]}...\n\n")
        ...
        if results:
            ...
            output_file = Path.home() / "Desktop" / f"batch_transcription_{timestamp}.txt"
            ...
            progress_text.insert(tk.END, f"Total: {sum(r['words'] for r in results)} words from {len(results)} files\n")
    
    threading.Thread(target=process_files, daemon=True).start()
    
    tk.Button(batch_win, text="Close", font=("Segoe UI", 11),
             bg="#4a9eff", fg="#ffffff", command=batch_win.destroy).pack(pady=10)
...
def save_to_history(text):
    """Save transcription to history with auto-backup."""
    global HISTORY, MAX_HISTORY, AUTO_SAVE_TRANSCRIPTIONS
    ...
    if AUTO_SAVE_TRANSCRIPTIONS:
        _auto_backup_history()


def _auto_backup_history():
    """Auto-backup history to dated file weekly."""
    global HISTORY
    ...
    # Only backup if file doesn't exist or is from previous week
...
def export_history():
    """Export history to multiple formats (TXT, JSON, Markdown, CSV)."""
    global HISTORY
    if not HISTORY:
    ...
    for fmt, label in formats:
        rb = tk.Radiobutton(popup, text=label, variable=selected_format, value=fmt,
    ...
    def do_export():
        popup.destroy()
        _export_to_format(selected_format.get())
...
def _export_to_format(format_type):
    """Internal function to export to specific format."""
    global HISTORY
...
LANGUAGE_SWITCHER_VISIBLE = False

def show_language_switcher():
    """Show a popup for quick language switching."""
    global LANGUAGE_SWITCHER_VISIBLE, LANGUAGE
    
    if LANGUAGE_SWITCHER_VISIBLE:
        return
    ...
    # Current language
    current_lang = LANGUAGE if LANGUAGE != "auto" else "auto-detect"
    tk.Label(popup, text=f"Current: {current_lang}", font=("Segoe UI", 10, "bold"),
    ...
    for code, name in languages:
        is_current = (code == LANGUAGE) or (code == "auto" and LANGUAGE == "auto")
        bg_color = "#4a9eff" if is_current else "#16213e"
        fg_color = "#ffffff"
    ...
    def close_popup():
        global LANGUAGE_SWITCHER_VISIBLE
        LANGUAGE_SWITCHER_VISIBLE = False
        popup.destroy()
